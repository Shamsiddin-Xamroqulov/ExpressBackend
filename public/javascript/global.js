...
async function handleDel(id) {
    const req = await fetch(`http://localhost:4000/api/action/delete/${id}`, {
        method: "DELETE",
        headers: {
...
async function getAndRenderActions() {
    ...
    try {
        const res = await fetch("http://localhost:4000/api/action/client", {
            method: "GET",
        ...
        if (res.ok) {
            ...
            actions.forEach(action => {
                ...
                if (action.status == 0) {
                    ...
                    statusBtn.textContent = "Klientga yuborishga tayyor !";
...
async function getEmployes() {
    try {
        const res = await fetch("http://localhost:4000/api/employes");
        if (res.ok) {
            const json = await res.json();
...
if (page === "/royhat.ishchilar.html") {
    ...
    if (renderBody && emploeTempEl) {
        ...
        async function handleCheckDel(dataId) {
            ...
            try {
                const req = await fetch(`http://localhost:4000/api/employes/delete/${targetId}`, {
                    method: "DELETE",
                    headers: {
                        "Content-type": "application/json",
                        token: acToken,
...
if (elActForm) {
    elActForm.addEventListener("submit", async (evt) => {
        ...
        const req = await fetch("http://localhost:4000/api/action/create", {
            method: "POST",
            headers: {
                "Content-type": "application/json",
                "token": acToken
